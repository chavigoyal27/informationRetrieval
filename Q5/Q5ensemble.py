"""
Combines: RoBERTa + TextBlob + VADER + Rule-based fixes
...
class EnsembleClassifier:
    ...
    def rule_based_fixes(self, text: str, ensemble_vote: str) -> tuple:
        ...
        if '?' in text and len(text.split()) > 3:
            return 'neutral', 0.7, 'question_override'
        ...
        for pattern in strong_positive:
            if re.search(pattern, text_lower):
                if ensemble_vote != 'positive':
                    return 'positive', 0.8, 'positive_override'
                break
        ...
        for pattern in strong_negative:
            if re.search(pattern, text_lower):
                if ensemble_vote != 'negative':
                    return 'negative', 0.8, 'negative_override'
                break

        return ensemble_vote, 0.6, 'no_override'

    def ensemble_vote(self, predictions: list) -> tuple:
        ...
        confidence = weights[final_label]
    ...
    def classify(self, text: str) -> dict:
        ...
        # Ensemble voting
        ensemble_label, ensemble_conf = self.ensemble_vote(predictions)

        # Apply rule-based overrides
        final_label, final_conf, override = self.rule_based_fixes(text, ensemble_label)
...
def evaluate_ensemble():
    ...
    # Load data
    df = pd.read_excel(r"C:\Users\Sutheerth\PycharmProjects\PythonProject2\eval.xls")
    ...
    for i, text in enumerate(texts):
        ...
        results['final'].append(result['final'])
